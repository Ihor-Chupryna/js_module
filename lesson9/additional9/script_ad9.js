...
for (const user of usersList) {
    ...
    for (const addressKey in user.address) {
        let addressElementDiv = document.createElement('div');
        addressElementDiv.innerText = user.address[addressKey];
        if (addressKey === 'geo') {
            continue;
        }
        addressDiv.appendChild(addressElementDiv);
    }
    ...
    for (const geoKey in user.address.geo) {
        ...
        geoDiv.innerText = user.address.geo[geoKey];
        addressGeoDiv.appendChild(geoDiv);
    }
    ...
    for (const companyKey in user.company) {
          let companyElementDiv = document.createElement('div')
